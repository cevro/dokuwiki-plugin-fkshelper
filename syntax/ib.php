...
class syntax_plugin_fkshelper_ib extends DokuWiki_Syntax_Plugin {
    ...
    public function connectTo($mode) {
        $this->Lexer->addSpecialPattern('{{ib.*?\>.+?\|.*?\|.*?}}', $mode, 'plugin_fkshelper_ib');
    }

    public function handle($match, $state) {

        $matches = [];
        preg_match('/{{ib(.*)>(.+?)}}/', $match, $matches);
        list(, $attributes, $p) = $matches;
        $attributes = $this->helper->matchClassesNIDs($attributes);
        $data = $this->helper->parseImageData($p);
        return array($state, [$data, $attributes, 'ib']);
    }
    ...
    public function render($mode, Doku_Renderer &$renderer, $data) {
        ...
        if ($mode == 'xhtml') {
            ...
            if ($data['image'] == null) {
                ...
                $this->helper->printImageDiv($renderer, $data['image']['id'], $data['label'], $data['href'], false, $imgSize, $param);
            }
        }
